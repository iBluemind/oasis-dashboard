...
function run_pep8 {
  ...
  DJANGO_SETTINGS_MODULE=oasis_dashboard.test.settings ${command_wrapper} flake8
}
...
function run_pep8_changed {
    ...
    warn_on_flake8_without_venv
    diff -u --from-file /dev/null ${files} | DJANGO_SETTINGS_MODULE=oasis_dashboard.test.settings ${command_wrapper} flake8 --diff
    exit
}

function run_sphinx {
    echo "Building sphinx..."
    DJANGO_SETTINGS_MODULE=oasis_dashboard.test.settings ${command_wrapper} python setup.py build_sphinx
    echo "Build complete."
}
...
function run_tests {
  ...
  if [ $with_selenium -eq 0 -a $integration -eq 0 ]; then
      testopts="$testopts --exclude-dir=oasis_dashboard/test/integration_tests"
  fi
...
function run_tests_all {
  ...
  export NOSE_XUNIT_FILE=oasis_dashboard/nosetests.xml
  if [ "$NOSE_WITH_HTML_OUTPUT" = '1' ]; then
    export NOSE_HTML_OUT_FILE='dashboard_nose_results.html'
  fi
  ${command_wrapper} ${coverage_run} $root/manage.py test oasis_dashboard --settings=openstack_dashboard.test.settings $testopts
...
function run_makemessages {
  ...
  echo -n "oasis dashboard : "
  cd oasis_dashboard
  babel_extract django
  MAGNUM_PY_RESULT=$?

  echo -n "oasis dashboard javascript: "
  babel_extract djangojs
  MAGNUM_JS_RESULT=$?

  cd ..
  if [ $check_only -eq 1 ]; then
    rm oasis_dashboard/locale/django*.pot
  fi

  exit $(($OASIS_PY_RESULT || $OASIS_JS_RESULT))
}
